...
// or manually via GET/POST for inspection.
...
async function runForecast() {
  ...
  for (const v of vendors) {
    ...
    const { error } = await admin
      .from("vendors")
      .update({ expected_customers: p.expected_customers, demand: p.demand } as any)
      .eq("id", v.id);
    if (!error) updated += 1;
  }

  return Response.json({ ok: true, updated, vendors: vendors.length, ai_used: predictions.length });
}
